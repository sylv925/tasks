...
export function bookEndList(numbers: number[]): number[] {
    ...
    let newArray: number[] = [numbers[0], numbers[numbers.length - 1]];
    return newArray;
}
...
export function stringsToIntegers(numbers: string[]): number[] {
    let newArray: number[] = numbers.map(
        (num: string): number => parseInt(num) || 0,
    );
    return newArray;
}
...
export const removeDollars = (amounts: string[]): number[] => {
    let newArray: number[] = amounts.map(
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    let newArray: string[] = messages.filter(
...
export function injectPositive(values: number[]): number[] {
    ...
    if (negativeIndex === -1) {
        return [
            ...values,
            values.reduce((sum: number, num: number): number => sum + num, 0),
        ];
    }
    let beforeNegative = values.slice(0, negativeIndex);
    let sumBeforeNegative = beforeNegative.reduce(
